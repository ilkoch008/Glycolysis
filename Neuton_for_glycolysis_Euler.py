...
def neuton(x0, y0, h, alpha, betta):
    ...
    while 1 > 0:
        matrix_J[0][0] = 1 + h * vector_sol[1]
        matrix_J[0][1] = vector_sol[0] * h
        matrix_J[1][0] = -h * alpha * vector_sol[1]
        matrix_J[1][1] = 1 - h*alpha*vector_sol[0] + alpha*betta*(betta+1)*h/math.pow(betta+vector_sol[1], 2)
        vector_at_point[0] = vector_sol[0] + h*vector_sol[0]*vector_sol[1] - x0 - h
        vector_at_point[1] = vector_sol[1] - h*alpha*vector_sol[0]*vector_sol[1] + h*alpha*vector_sol[1]*(1+betta)/(vector_sol[1]+betta) - y0
